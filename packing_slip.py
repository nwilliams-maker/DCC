...
# Read-only campaign lookup against manage.terraboost.com (populates SIO + art-file
...
def _strip_campaign_dates(s: str) -> str:
    ...
    # NOTE: we DO NOT strip trailing National/Local/Regional tags from the
    # campaign string — the customer-type detection downstream depends on the
    # word "National" appearing in the campaign name. Stripping it caused
    # every National row to fall through to "Local" bucket. Leaving as-is.
...
def render_packing_slip_button(
    ...
    *,
    ...
    html = (
        ...
        + ''  # jsPDF loaded lazily on first button click — see _loadJsPdf below
        + '<script>(function(){\n'
        ...
        + r"""
  // Lazy-load jsPDF on first click. Each iframe maintains its own promise;
  // the browser caches the CDN script after the first iframe fetches it,
  // so subsequent loads in the same page are near-instant.
...
"""
  ...
  async function _onClick(ev) {
    try { await _loadJsPdf(); } catch (e) { console.error(e); alert('Packing slip generator failed to load. Try again.'); return; }
    ev.preventDefault();
